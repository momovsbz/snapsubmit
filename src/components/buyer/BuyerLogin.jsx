...
export default function BuyerLogin({ onLogin }) {
  ...
  return (
    <div className="min-h-screen bg-[#f9fafb] flex items-center justify-center px-4">
      ...
      >
        <div className="w-14 h-14 bg-[#d1f2ef] border border-[#0e746a]/20 rounded-2xl flex items-center justify-center mx-auto mb-5">
          <Lock className="w-6 h-6 text-[#0e746a]" />
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <input
            ...
            autoComplete="off"
            className="w-full bg-[#f9fafb] border border-[#e5e7eb] rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0e746a]/30"
          />
          <input
            type="password"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#0e746a] text-white font-semibold py-2.5 rounded-lg text-sm hover:bg-[#0c5f57] transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
          >
