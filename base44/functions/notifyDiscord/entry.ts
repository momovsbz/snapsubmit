...
Deno.serve(async (req) => {
  try {
    ...
    // Post message via Bot Token to create thread-capable message
    const postRes = await fetch(`https://discord.com/api/v10/channels/${DISCORD_CHANNEL_ID}/messages`, {
      method: "POST",
      headers: {
    ...
    const reactionRes = await fetch(
      `https://discord.com/api/v10/channels/${DISCORD_CHANNEL_ID}/messages/${messageId}/reactions/%E2%9C%85/@me`,
      {
        method: "PUT",
        headers: {
          "Authorization": `Bot ${DISCORD_BOT_TOKEN}`,
    ...
    if (!reactionRes.ok) {
      const error = await reactionRes.text();
      console.error(`Discord reaction error: ${error}`);
    }

    // Update submission with discord_message_id
