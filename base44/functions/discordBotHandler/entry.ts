...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (interaction.type === 3) {
      ...
      // Mettre à jour la soumission avec le code
      await base44.asServiceRole.entities.Submission.update(submissionId, { code, status: 'code_ready' });

      // Créer un DM channel
      const dmResponse = await fetch(`https://discord.com/api/v10/users/${userId}/channels`, {
        method: 'POST',
        headers: {
      ...
      if (dmResponse.ok) {
        ...
        await fetch(`https://discord.com/api/v10/channels/${dmChannel.id}/messages`, {
          ...
          body: JSON.stringify({
            content: `✅ **Code Snapchat+ pour @${snapchat}**\n\n🔑 **Code:** \`${code}\`\n\nCe code est valide pendant 24h.`
          })
        });
      }
